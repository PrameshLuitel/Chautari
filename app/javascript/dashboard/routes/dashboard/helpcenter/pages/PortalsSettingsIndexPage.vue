...
const { isOnChautariCloud } = useAccount();
...
const fetchSSLStatus = () => {
  if (!isOnChautariCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on Chautari cloud)
  if (portalObj?.custom_domain && isOnChautariCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
