...
const latestChautariVersion = computed(() => {
...
const hasAnUpdateAvailable = computed(() => {
  if (!semver.valid(latestChautariVersion.value)) {
    return false;
  }

  return semver.lt(globalConfig.value.appVersion, latestChautariVersion.value);
});
    ...
    <div v-if="hasAnUpdateAvailable && globalConfig.displayManifest">
      {{
        t('GENERAL_SETTINGS.UPDATE_CHATWOOT', {
          latestChautariVersion: latestChautariVersion,
        })
      }}
    </div>
