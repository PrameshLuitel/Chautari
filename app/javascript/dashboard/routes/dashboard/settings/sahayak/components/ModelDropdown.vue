...
const PROVIDER_ICONS = {
  ...
  groq: 'i-lucide-cpu', // Placeholder for Groq
};

const iconForModel = model => {
  return PROVIDER_ICONS[model.provider] || 'i-lucide-box';
};
...
const availableModels = computed(() =>
  sahayakConfigStore
    .getModelsForFeature(props.featureKey)
    .filter(m => m.provider === 'groq')
);

const recommendedModelId = computed(() =>
  sahayakConfigStore.getDefaultModelForFeature(props.featureKey)
);

const selectedModel = computed(() =>
  sahayakConfigStore.getSelectedModelForFeature(props.featureKey)
);
...
const getCreditLabel = model => {
  ...
  return t('SAHAYAK_SETTINGS.MODEL_CONFIG.CREDITS_PER_MESSAGE', {
    credits: multiplier,
  });
};
  ...
  <div v-on-clickaway="closeDropdown" class="relative flex-shrink-0">
    ...
    >
      ...
      <span v-else class="text-n-slate-10">
        {{ t('SAHAYAK_SETTINGS.MODEL_CONFIG.SELECT_MODEL') }}
      </span>
      <Icon
        ...
        <div class="flex gap-2 w-full">
          ...
          <div class="flex flex-col w-full text-left gap-1">
            ...
            <span v-if="model.coming_soon" class="text-xs text-n-slate-11">
              {{ t('SAHAYAK_SETTINGS.MODEL_CONFIG.COMING_SOON') }}
            </span>
